...
단순 경로 존재 유무 메서드는 Sync API인 `existsSync()` 만 사용된다. Promise-based API에는 존재하지 않고 Callback API인 `exists()`는 deprecated 되었다.

비동기로 파일 존재를 확인하려면 아래의 `access()`를 쓰는 것이 좋다.
...
주의할 점으로 파일을 읽거나 쓰기 전 접근성 확인을 위해 이 메서드를 사용해서는 안된다. 접근성 확인 후 파일을 열기 전 다른 프로세스가 해당 파일에 접근할 수 있기 때문에 파일을 읽거나 쓰는 메서드에서 발생하는 에러를 처리하는 방식으로 접근성 확인을 수행해야 한다.
...
fs.cp('source.txt', 'destination.txt', (err) => {
    if (err) throw err;
});
```
...
- recursive : 디렉토리 재귀적으로 복사할 지 여부 (기본값: `false`)
